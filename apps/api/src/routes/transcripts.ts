...
transcriptsRouter.post("/", async (c) => {
  ...
  const existing = await findJobByIdempotencyKey(c.env.DB, user.id, idempotencyKey.data);
  if (existing) {
    if (existing.state === "queued") {
      await c.env.GENERATION_QUEUE.send({ jobId: existing.id, userId: user.id, videoId: await getJobVideoId(c.env.DB, existing.id, user.id) });
    }
    return c.json(
      TranscriptUploadResponseSchema.parse({ jobId: existing.id, stage: existing.stage }),
      202,
    );
  }
  ...
  try {
    ...
    const raced = await findJobByIdempotencyKey(c.env.DB, user.id, idempotencyKey.data);
    if (raced) {
      return c.json(TranscriptUploadResponseSchema.parse({ jobId: raced.id, stage: raced.stage }), 202);
    }
    throw error;
  }
  const status = TranscriptUploadResponseSchema.parse({ jobId, stage: "creating_questions" });
  await c.env.GENERATION_QUEUE.send({ jobId, userId: user.id, videoId: input.videoId } satisfies GenerationQueueMessage);
...
generationRouter.post("/:jobId/retry", async (c) => {
  ...
  if (job.cancel_requested || job.error_code === "generation_cancelled") {
    throw new ApiError(409, "generation_cancelled", "This generation was cancelled. Start a new quiz instead.");
  }
  if (job.state === "failed") {
    ...
    ).bind(now(), job.id, user.id).run();
  }
  const queuedStatus = GenerationStatusSchema.parse({ jobId: job.id, stage: "creating_questions", progress: 0, quizId: null, error: null });
  await c.env.GENERATION_QUEUE.send({ jobId: job.id, userId: user.id, videoId: await getJobVideoId(c.env.DB, job.id, user.id) });
...
generationRouter.delete("/:jobId", async (c) => {
  ...
  if (job.state === "complete") {
    throw new ApiError(409, "generation_complete", "This quiz has already been created.");
  }
  const status = GenerationStatusSchema.parse({
    ...
    progress: 1,
    quizId: null,
    error: { code: "generation_cancelled", message: "Quiz creation was cancelled." },
  });
  await c.env.DB.prepare(
...
async function findOwnedJob(db: D1Database, jobId: string, userId: string): Promise<JobRow | null> {
  return db.prepare(
...
async function findJobByIdempotencyKey(db: D1Database, userId: string, key: string): Promise<JobRow | null> {
  return db.prepare(
  ...
  ).bind(userId, key).first<JobRow>();
}

async function getJobVideoId(db: D1Database, jobId: string, userId: string): Promise<string> {
  const row = await db.prepare("SELECT video_id FROM generation_jobs WHERE id = ? AND user_id = ?")
    .bind(jobId, userId)
    .first<{ video_id: string }>();
  if (!row) throw new ApiError(404, "generation_not_found", "Generation job not found.");
  return row.video_id;
}
