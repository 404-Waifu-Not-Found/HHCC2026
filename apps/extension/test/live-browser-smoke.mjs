...
try {
  ...
  if (videoId) {
    ...
    extractionPageCount = context.pages().length;
  }
  ...
  if (videoId) {
    ...
    if (context.pages().length !== extractionPageCount) {
      throw new Error(
        `Caption extraction changed the visible tab count from ${extractionPageCount} to ${context.pages().length}.`,
      );
    }
    console.log("Caption extraction completed without opening another tab.");
  }
} finally {
  await context.close();
