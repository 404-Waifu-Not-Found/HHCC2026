...
export async function transcribeLocally(
  ...
  if (options.durationSeconds > 5_400)
    throw new Error("Captionless videos can be at most 90 minutes.");
  ...
  const tempDirectory = new Directory(
    Paths.cache,
    `clipquest-transcription-${options.videoId}`,
  );
  ...
  try {
    ...
    throwIfAborted(options.signal);
    const chunks = await LocalAudioDecoder.decodeToChunks(
      audioFile.uri,
      chunkDirectory.uri,
      30,
      5,
    ...
    try {
      ...
      for (
        let index = checkpoint.completedChunks;
        index < chunks.length;
        index += 1
      ) {
        ...
        const transcription = context.transcribe(chunk.uri, {
          ...
          onProgress: (progress) =>
            options.onProgress((index + progress / 100) / chunks.length),
        });
        const abort = () => {
        ...
        if (result.isAborted || options.signal.aborted)
          throw new TranscriptionPausedError();
        segments.push(
          ...result.segments.flatMap((segment, segmentIndex) => {
            if (!segment.text.trim() || (index > 0 && segment.t1 <= 500))
              return [];
            const startMs =
              chunk.startMs + Math.max(index > 0 ? 5_000 : 0, segment.t0 * 10);
            const endMs = Math.max(
              startMs + 50,
              chunk.startMs + segment.t1 * 10,
            );
            return [
              {
                id: `device-${index}-${segmentIndex}`,
                startMs,
                endMs,
...
export async function getLocalModelStatus(): Promise<ModelStatus> {
  ...
  return {
    cached:
      MODEL_FILE.exists &&
      (expectedSize === null || MODEL_FILE.size === expectedSize),
...
async function ensureModel(
  ...
  if (MODEL_FILE.exists && MODEL_FILE.size === manifest.native.file.sizeBytes) {
    ...
    options.onProgress(1, {
      loadedBytes: MODEL_FILE.size,
      totalBytes: MODEL_FILE.size,
      cached: true,
    });
    return MODEL_FILE;
  }
  ...
  const onProgress = ({
    ...
    options.onProgress(
      totalBytes > 0
        ...
        : bytesWritten / manifest.native.file.sizeBytes,
      {
        loadedBytes: bytesWritten,
        totalBytes:
          totalBytes > 0 ? totalBytes : manifest.native.file.sizeBytes,
      },
    );
  };
  ...
  if (pausedRaw) {
    try {
      task = DownloadTask.fromSavable(
        JSON.parse(pausedRaw) as DownloadPauseState,
        { onProgress },
      );
