...
async function acquireTextTranscriptOnce(
  ...
  try {
    onProgress?.(0.3);
    const startedAt = Date.now();
    const transcript = await downloadDirectYouTubeTranscript(
      imported.video.sourceVideoId,
      signal,
      preferredLanguage ?? imported.video.sourceLanguage ?? "en",
    );
    if (
      imported.video.durationSeconds > 0 &&
      transcript.durationSeconds &&
      ...
      throw new Error(
        "The direct caption duration did not match the imported YouTube video.",
      );
    }
    ...
    console.info(
      JSON.stringify({
        scope: "caption_acquisition",
        event: "youtube_direct.completed",
        sourceVideoId: imported.video.sourceVideoId,
        isAutoGenerated: transcript.isAutoGenerated,
        sourceSegmentCount: transcript.sourceSegmentCount,
        segmentCount: transcript.segments.length,
