...
export async function transcribeLocally(
  ...
  if (!canTranscribeInBrowser(options.durationSeconds)) {
    throw new Error(
      "Captionless videos can be at most 90 minutes in a browser. Use the mobile app for longer videos.",
    );
  }
  ...
  throwIfAborted(options.signal);
  const checkpoint = await loadCheckpoint(options.ownerUserId, options.videoId);
  const segments = [...checkpoint.segments];
  const worker = new Worker("/whisper-worker.js", {
    type: "module",
    name: "clipquest-whisper",
  });
      ...
      worker.onmessage = (event: MessageEvent<WorkerMessage>) => {
        ...
        if (message.type === "phase") {
          ...
          void AsyncStorage.setItem(
            checkpointKey(options.ownerUserId, options.videoId),
            JSON.stringify({ completedChunks, segments } satisfies Checkpoint),
          );
...
export async function getLocalModelStatus(): Promise<ModelStatus> {
  ...
  if (!("caches" in globalThis))
    return { cached: false, sizeBytes: manifest?.web.sizeBytes ?? null };
  const cache = await caches.open("clipquest-whisper-v1");
  const keys = await cache.keys();
  const cached = Boolean(
    manifest &&
    manifest.web.files.every((file) =>
      keys.some((request) => request.url.endsWith(`/${file.path}`)),
    ),
  );
...
export async function removeLocalModel(): Promise<boolean> {
  ...
  await AsyncStorage.multiRemove(
    keys.filter((key) => key.startsWith("clipquest:transcript-checkpoint:")),
  );
  return removed;
}
...
async function downloadMedia(
  ...
  if (!response.ok) {
    throw new Error(
      await readResponseErrorMessage(
        response,
        `Audio delivery failed (${response.status}).`,
      ),
    );
  }
  if (!response.body) throw new Error("Audio delivery returned no data.");
  const total = Number(response.headers.get("content-length") ?? 0);
  if (Number.isFinite(total) && total > MAX_MEDIA_BYTES) {
  ...
  while (true) {
    ...
    if (loaded > MAX_MEDIA_BYTES) {
      await reader.cancel();
      throw new Error("The video audio is too large to transcribe safely.");
    }
    onProgress(
      total > 0 ? loaded / total : Math.min(0.95, loaded / 20_000_000),
    );
  }
...
async function decodeAudio(
  ...
  if (contentType.includes("mp4") && "AudioDecoder" in globalThis) {
    try {
      return await decodeMp4WithWebCodecs(buffer.slice(0));
    } catch {
      /* Browser codec/container support varies. */
    }
  }
  ...
  if (!Context)
    throw new Error("This browser cannot decode the video's audio.");
...
async function decodeMp4WithWebCodecs(
  buffer: ArrayBuffer,
): Promise<Float32Array> {
  ...
  const decoder = await new Promise<AudioDecoder>((resolve, reject) => {
    ...
    file.onReady = async (info) => {
      ...
      if (!support.supported)
        return reject(new Error("Audio codec is not supported by WebCodecs."));
        ...
        output: (audio) => {
          ...
          for (let index = 0; index < audio.numberOfChannels; index += 1) {
            ...
            for (let frame = 0; frame < mono.length; frame += 1)
              mono[frame] =
                (mono[frame] ?? 0) +
                (channel[frame] ?? 0) / audio.numberOfChannels;
          }
          frames.push(mono);
          audio.close();
        },
...
function downmix(audio: AudioBuffer): Float32Array {
  ...
  for (let channel = 0; channel < audio.numberOfChannels; channel += 1) {
    ...
    for (let index = 0; index < mono.length; index += 1)
      mono[index] =
        (mono[index] ?? 0) + (data[index] ?? 0) / audio.numberOfChannels;
  }
  return mono;
}
...
async function loadCheckpoint(
  ...
  const raw = await AsyncStorage.getItem(checkpointKey(ownerUserId, videoId));
  if (!raw) return { completedChunks: 0, segments: [] };
  try {
    return JSON.parse(raw) as Checkpoint;
  } catch {
    return { completedChunks: 0, segments: [] };
  }
}
