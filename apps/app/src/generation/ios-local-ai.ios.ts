...
export async function readIosDeepSeekKey(
  userId: string,
): Promise<string | null> {
  return SecureStore.getItemAsync(accountKey(userId));
}

export async function saveIosDeepSeekKey(
  userId: string,
  apiKey: string,
): Promise<void> {
...
export async function removeIosDeepSeekKey(userId: string): Promise<void> {
  await SecureStore.deleteItemAsync(accountKey(userId));
  listeners.forEach((listener) => listener(userId, false));
}

export function subscribeToIosDeepSeekKey(
