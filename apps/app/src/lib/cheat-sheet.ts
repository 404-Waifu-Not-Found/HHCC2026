...
import clipquestIconAsset from "../../assets/platform/app-icon-1024.png";
...
export async function renderCheatSheetPdf(
  ...
  const clipquestIcon = await pdf.embedPng(
    await loadAssetBytes(clipquestIconAsset),
  );
  ...
  const width = 516;
  const footer = () => {
    page.drawImage(clipquestIcon, {
      x: margin,
      y: 22,
      width: 13,
      height: 13,
    });
    page.drawText("Generated by ClipQuest", {
      x: margin + 18,
      y: 25,
      size: 8,
      font,
  ...
  write(
    `Source: ${document.source} · Generated ${formatGeneratedDate(document.generatedAt)}`,
    9,
    false,
    rgb(0.35, 0.4, 0.37),
  );
...
async function loadUnicodeFont(): Promise<Uint8Array> {
  return loadAssetBytes(unicodeFontAsset);
}
