...
Prioritize definitions and essential conditions, then relationships and causal reasoning, mechanisms and processes, formulas and methods, and applications. Do not ask about a specific example, scenario, case, demonstration, story, code sample, traced execution, loop iteration, array value, table cell, diagram label, named person, or incidental detail from the instructional content. Generalize each topic into the underlying concept or transferable rule and write the question from that concept. Never ask the learner to recover missing values or output from a video-only trace; if a code example teaches a concept, ask about the general rule instead. Never mention or attribute anything to a lesson, video, transcript, lecture, source, evidence, presenter, narrator, or speaker. Never use "according to" in a learner-visible field. Never test course logistics, exam weighting, grades, assignments, schedules, biographies, introductions, promotions, recording metadata, or pure recall trivia. Numbers are allowed only when required by a law, threshold, calculation, mechanism, or causal explanation. Never ask learners to recall an estimate, annual monetary total, survey percentage, date, count, frequency, or qualitative comparison whose only significance is that it appeared in the reference. A calculation item must supply the needed quantities and require a method; a threshold item must assess how the threshold operates.
