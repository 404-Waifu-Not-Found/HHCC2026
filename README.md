...
As of **2026-08-11**, this source tree implements evidence-grounded automatic-recovery progressive extension-local quiz streaming. The v5.4 profile remains disabled by default until its benchmark and canary gates pass. The live `/health` response and Wrangler deployment history remain authoritative for [clipquest.ccwu.cc](https://clipquest.ccwu.cc); a local commit is not evidence that production has been deployed.

- When enabled, new banks use extension `0.8.4`, result protocol `8`, capability `question-stream-v4`, pipeline `9`, prompt `quiz-local-json-stream-v5.4`, validator `validator-local-progressive-v4.3`, and progressive import `v6`.
- Extension `0.8.4` retains isolated protocol-7/v5.3, protocol-6/v5.2, and protocol-5/v5.1 compatibility paths; completed pipeline-7 and existing pipeline-9 banks remain readable without mixing generation metadata.
...
- Every v5.4 question carries a locally verified source-evidence excerpt and semantic claim key. True/False answers are constructed from exact supported statements or deterministic verified mutations, while multiple-choice mappings and equivalent-choice rejection are handled locally.
...
For an evidence-grounded automatic-recovery bank, the site sends the verified in-memory quiz context to extension `0.8.4` through result protocol `8`. The extension sends the complete plain text directly to DeepSeek V4 Flash with thinking disabled, temperature `0.2`, `stream: true`, JSON-object response mode, and a 4,096-token singleton output limit. Its SSE parser tolerates UTF-8 and CRLF boundaries, keep-alive comments, usage-only chunks, escaped braces and quotes, truncation, and `[DONE]`. The transcript and stable instructions remain a byte-identical prefix while only the current slot, instructional excerpt, accepted claim summaries, and repair guidance change. A disabled v5.4 rollout continues through the isolated v5.3, v5.2, or v5.1 compatibility profile selected by the Worker.
...
- Evidence-grounded automatic generation uses result protocol `8`, request IDs, extension-owned ports, exact-origin checks, payload bounds, cancellation, and timeouts. Extension `0.8.4` must advertise `question-stream-v4`; protocols `5`/`6`/`7` and capabilities `question-stream-v1`/`v2`/`v3` remain compatibility paths only. The API key is never part of any protocol.
