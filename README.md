...
As of **2026-08-13**, this source tree implements concept-first automatic-only progressive extension-local quiz streaming. Extension 0.8.13 and prompt v5.8 are the current canary candidate; general rollout remains blocked until official-site acceptance passes. The live `/health` response and Wrangler deployment history remain authoritative for [clipquest.ccwu.cc](https://clipquest.ccwu.cc); a local commit is not evidence that production has been deployed.

- Canary banks use extension `0.8.13`, result protocol `9`, capability `question-stream-v6`, pipeline `9`, prompt `quiz-local-json-stream-v5.8`, validator `validator-local-progressive-v4.12`, and progressive import `v7`.
...
- The current concept-first path has no repair mode and no learner continuation control. Each ordinal uses a singleton primary request followed, only when needed, by up to four bounded automatic retries; the bank-wide ceiling is 48 extra model calls and 15 active recovery minutes. Existing v5.4-v5.7 banks keep their original compatibility limits. Credential and billing failures require configuration instead of blind retries.
