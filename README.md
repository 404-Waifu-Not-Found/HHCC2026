...
As of **2026-08-13**, this source tree contains the prompt-first, minimal-validation successor to the existing concept-first generator. Extension 0.8.14 and prompt v5.9 are a local candidate behind a disabled v5.9 rollout flag; they are not production evidence until the direct benchmark and official-site canary pass. The live `/health` response and Wrangler deployment history remain authoritative for [clipquest.ccwu.cc](https://clipquest.ccwu.cc).

- New prompt-first banks use extension `0.8.14`, result protocol `10`, capability `question-stream-v7`, pipeline `9`, prompt `quiz-local-json-stream-v5.9`, validator `validator-minimal-structural-v5.0`, progressive import `v8`, and profile `prompt_first_auto_v5_9` only when the new rollout is assigned.
- The fallback production profile remains extension `0.8.13`, protocol `9`, capability `question-stream-v6`, prompt v5.8, and validator v4.12 until v5.9 is deliberately canaried.
...
- Prompt v5.9 receives one preselected answer-bearing instructional window, one exact type-specific schema, and accepted question/concept summaries. Runtime validation is limited to storage and grading integrity; editorial quality is audited in tests and release QA rather than through model retry loops. Existing protocol-5 through protocol-9 banks retain their original validators and recovery semantics.
