...
As of **2026-08-18**, this source tree adds the native Android private-beta client while preserving the current prompt-first web-generation contracts. Android 0.2.0 and Chrome extension 0.8.19 consume one shared local quiz engine. This is source and local-emulator evidence, not proof of an EAS-signed private APK or completed physical-device acceptance. The live `/health` response and Wrangler deployment history remain authoritative for [clipquest.ccwu.cc](https://clipquest.ccwu.cc).
