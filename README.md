...
- Caption segments and normalized text remain inside the local Chrome/Android generation client. Progressive import receives only video/session settings, verified aggregate source metadata, bounded generation metadata, and one validated question at a time. Android's bounded outbox contains only validated questions and safe events—not a key, caption text, prompt, or model body.
